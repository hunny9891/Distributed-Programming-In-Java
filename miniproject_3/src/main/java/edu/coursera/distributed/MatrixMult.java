...
public class MatrixMult {
     ...
     * program. For simplicity MPI_Init has already been called, and MPI_Finalize
     * should not be called in parallelMatrixMultiply.
     *
     * On entry to parallelMatrixMultiply, the following will be true of a, b, and
     * c:
     *
     * 1) The matrix a will only be filled with the input values on MPI rank zero.
     * Matrix a on all other ranks will be empty (initialized to all zeros). 2)
     * Likewise, the matrix b will only be filled with input values on MPI rank
     * zero. Matrix b on all other ranks will be empty (initialized to all zeros).
     ...
     *
     * 1) On rank zero, matrix c must be filled with the final output of the full
     * matrix multiplication. The contents of matrix c on all other ranks are
     * ignored.
     *
     * Therefore, it is the responsibility of this method to distribute the input
     * data in a and b across all MPI ranks for maximal parallelism, perform the
     * matrix multiply in parallel, and finally collect the output data in c from
     * all ranks back to the zeroth rank. You may use any of the MPI APIs provided
     * in the mpi object to accomplish this.
     *
     * A reference sequential implementation is provided below, demonstrating the
     * use of the Matrix class's APIs.
     *
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c, final MPI mpi) throws MPIException {
        ...
        // For simplicity we will broadcast all the matrices a & b to all ranks (should
        // not be done in real
        // production environment.)
        // More efficient approach is to send relevant rows and cols to different ranks.
